...
from lava.readers.system_execution import (
    bootstrap,
    ensure_job,
    job_name,
    training_request,
...
def asset(number=1, document=1):
    return OraclePageAsset(
        asset_version="test",
        document_id=f"private-pdf-{document:02d}",
        document_alias=f"doc-{document:02d}",
        page_number=number,
        source_pdf_s3_uri="s3://test/source.pdf",
        source_pdf_sha256="a" * 64,
        image_s3_uri="s3://test/image.png",
        image_sha256="b" * 64,
        text_s3_uri="s3://test/text.txt",
        text_sha256="c" * 64,
        layout_s3_uri="s3://test/layout.json",
        layout_sha256="d" * 64,
        width_pixels=100,
        height_pixels=100,
        dpi=180,
...
def request(index=1, pages=(1, 3, 5, 7, 9)):
    ...
    return ReaderInput(
        question_id=f"private-question-{index:02d}",
        document_id=f"private-pdf-{document:02d}",
        document_alias=f"doc-{document:02d}",
        question="Private question text",
        answer_format="string",
        language="ja",
        pages=tuple(asset(number, document) for number in pages),
    )
...
def telemetry(images=5):
    return ReaderTelemetry(
        ...
        generated_tokens=2,
        image_count=images,
        total_image_pixels=images * 10000,
        raw_response_characters=100,
...
def fixture():
    ...
    raws = {
        row["question_id"]: '{"answer":"x","evidence_pages":[1],"confidence":0.5,"abstain":false}'
        for row in inputs
    }
    ...
    def predict(row):
        return parse_reader_response(
            ...
            raw_response=raws[row.question_id],
            allowed_pages=row.available_pages,
        ), telemetry()
...
def test_existing_oracle_contract_remains_gold_aligned_and_four_page_bounded():
    ...
    original = OracleExample(
        protocol_lock_id="a" * 64, answer="private label", evidence_pages=(1,), **row.model_dump()
    )
    ...
    with pytest.raises(ValueError):
        OracleExample(
            protocol_lock_id="a" * 64, answer="label", evidence_pages=(2,), **row.model_dump()
        )
    with pytest.raises(ValueError):
        OracleExample(
            protocol_lock_id="a" * 64,
            answer="label",
            evidence_pages=(1, 3, 5, 7, 9),
            **request().model_dump(),
        )
...
@pytest.mark.parametrize(
    "order,budget",
    [([1, 1], 1), ([0, 1], 1), ([1, 3], 1), ([True, 2], 1), ([1, 2], True), ([1, 2], 0)],
)
...
def test_input_manifest_rejects_partial_corrupt_and_label_bearing_inputs(mutation):
    ...
    if mutation == "contract":
        contract = {**contract, "contract_id": "2" * 64}
    if mutation == "label":
...
def test_interrupted_inference_resumes_only_unfinished_questions_and_all_cached_skips_model(capsys):
    ...
    assert (
        infer_requests(manifest, contract, store, forbidden, logger, read_raw=forbidden) == result
    )
...
def test_invalid_model_output_is_a_completed_counted_failure_not_silently_retried():
    ...
    prediction = ReaderPrediction(
        ...
        abstain=True,
        schema_valid=False,
        parser_error_code=error.value.code,
        raw_response_sha256=digest(raw.encode()),
    )
    saved = {
        "input_sha256": digest(encode(row.model_dump(mode="json"))),
        "raw_response": raw,
...
def test_scoring_uses_predicted_not_gold_pages_and_preserves_all_denominators():
    ...
    assert (
        "private-question" not in public
        and "Private question" not in public
        and "private-pdf" not in public
    )
    with pytest.raises(ValueError):
        score_predictions(
            manifest, {**result, "records": result["records"][:-1]}, refs, NormalizedExactJudge()
        )
...
def test_reader_verifies_asset_bytes_versions_and_closes_stream(bad_sha, bad_version):
    ...
    reader.s3 = Mock()
    reader.s3.get_object.return_value = {
        "Body": stream,
        "VersionId": "wrong" if bad_version else "v1",
    }
    if bad_sha or bad_version:
        with pytest.raises(ValueError):
            reader._get_bytes(
                "s3://test/image.png",
                expected_sha256="a" * 64 if bad_sha else digest(b"image"),
                version_id="v1",
            )
    else:
        assert (
            reader._get_bytes(
                "s3://test/image.png", expected_sha256=digest(b"image"), version_id="v1"
            )
            == b"image"
        )
...
def test_job_request_passes_botocore_contract_and_enforces_cloud_limits():
    ...
    assert request_body["StoppingCondition"] == {
        "MaxRuntimeInSeconds": 1800,
        "MaxPendingTimeInSeconds": 86400,
    }
    assert all(
        len(arg) <= 256 for arg in request_body["AlgorithmSpecification"]["ContainerArguments"]
    )
...
@pytest.mark.parametrize(
    "identity,attempt", [("x", 1), ("a" * 64, 0), ("a" * 64, True), ("a" * 64, 100)]
)
def test_job_name_rejects_invalid_identity_and_attempt(identity, attempt):
    with pytest.raises(ValueError):
        job_name(identity, attempt)
...
def test_paid_job_cannot_be_created_without_explicit_acknowledgement(retry, attempt):
    ...
    client.describe_training_job.side_effect = ClientError(
        {"Error": {"Code": "ResourceNotFound"}}, "DescribeTrainingJob"
    )
    with pytest.raises(ValueError):
        ensure_job(
            client,
            {"TrainingJobName": "test"},
            acknowledge_charges="NO",
            allow_retry=retry,
            attempt=attempt,
...
def test_ambiguous_create_timeout_reattaches_instead_of_creating_another_job():
    ...
    client.describe_training_job.side_effect = [
        missing,
        {**body, "TrainingJobStatus": "InProgress"},
    ]
    client.get_paginator.return_value.paginate.return_value = [{"TrainingJobSummaries": []}]
    client.create_training_job.side_effect = EndpointConnectionError(
        endpoint_url="https://sagemaker.test"
    )
    result = ensure_job(
        client,
        body,
...
def test_remote_progress_does_not_expose_arbitrary_log_fields(capsys):
    ...
    client = Mock()
    record = {
        "event": "system.question.completed",
        "completed": 3,
        "total": 16,
        "raw_response": "PRIVATE_ANSWER",
...
def test_preparation_reuses_real_pdf_extraction_ranking_and_rendered_assets(tmp_path, monkeypatch):
    ...
    class S3:
        ...
        def put_object(self, *, Key, Body, Metadata, **kwargs):
            ...
            if "IfMatch" in kwargs and (
                existing is None or kwargs["IfMatch"] != digest(existing[0])
            ):
                raise ClientError({"Error": {"Code": "PreconditionFailed"}}, "PutObject")
    ...
    writer.writerow(
        ["id", "file_id", "question", "answer_format", "answer", "evidence_page_number", "language"]
    )
    ...
    monkeypatch.setattr(
        system, "_render_page", Mock(side_effect=AssertionError("No duplicate rendering"))
    )
    monkeypatch.setattr(
        system, "extract_pages", Mock(side_effect=AssertionError("No duplicate extraction"))
    )
    monkeypatch.setattr(
        system, "rank_query", Mock(side_effect=AssertionError("No duplicate ranking"))
    )
