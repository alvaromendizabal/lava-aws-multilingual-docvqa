...
def job_name(identity: str, attempt: int) -> str:
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", identity) or type(attempt) is not int or not 1 <= attempt <= 99:
...
def training_request(
    root: Path, manifest: dict[str, Any], bucket: str, region: str, role: str,
    attempt: int, source_key: str, source_sha: str, commit: str,
) -> dict[str, Any]:
    """One pinned image, one GPU, a server-enforced runtime cap, and no endpoint."""
    contract = manifest["contract"]
    validate_manifest(manifest, contract)
    config = contract["config"]
    runtime = yaml.safe_load((root / "configs/oracle_reader_benchmark.yaml").read_bytes())["training_runtime"]
    ...
    return {
        ...
        "InputDataConfig": [{
            ...
            "DataSource": {"S3DataSource": {
                "S3DataType": "S3Prefix", "S3Uri": f"s3://{bucket}/{source_key}",
                "S3DataDistributionType": "FullyReplicated",
            }},
            "CompressionType": "None", "InputMode": "File",
        }],
        "OutputDataConfig": {
            "S3OutputPath": f"s3://{bucket}/experiments/oracle-reader/system-runs/{identity}/attempt-{attempt}",
            "CompressionType": "NONE",
        },
        "ResourceConfig": {"InstanceType": "ml.g6e.2xlarge", "InstanceCount": 1, "VolumeSizeInGB": 100},
        "StoppingCondition": {
        ...
        "Environment": {
            ...
            "PYTHONHASHSEED": "20260902", "LAVA_GIT_COMMIT_SHA": commit,
            "AWS_DEFAULT_REGION": region, "LAVA_BUCKET": bucket,
            "LAVA_SYSTEM_CONTRACT": identity, "LAVA_SOURCE_SHA256": source_sha,
        },
        "Tags": [{"Key": "Project", "Value": "lava-docvqa"},
...
def describe_or_none(client: Any, name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if details.get("Code") == "ResourceNotFound" or (
            details.get("Code") == "ValidationException" and
...
def validate_remote(description: dict[str, Any], request: dict[str, Any]) -> None:
    ...
    for name in ("TrainingJobName", "RoleArn", "ResourceConfig", "StoppingCondition", "Environment"):
    ...
    if description["InputDataConfig"][0]["DataSource"] != request["InputDataConfig"][0]["DataSource"]:
        raise ValueError("Existing training job has different source data")


def ensure_job(
    client: Any, request: dict[str, Any], *, acknowledge_charges: str, allow_retry: bool,
    attempt: int, logger: RuntimeEventLogger,
) -> dict[str, Any]:
    ...
    for state in ("InProgress", "Stopping"):
        for page in client.get_paginator("list_training_jobs").paginate(StatusEquals=state):
            if any("lava" in row["TrainingJobName"].lower() for row in page["TrainingJobSummaries"]):
...
def stream_progress(logs: Any, name: str, logger: RuntimeEventLogger, seen: set[str]) -> None:
    ...
    try:
        pages = logs.get_paginator("filter_log_events").paginate(
            logGroupName="/aws/sagemaker/TrainingJobs", logStreamNamePrefix=name,
        ...
        for page in pages:
            for event in page.get("events", []):
                ...
                try:
                    ...
                    continue
                if not isinstance(record, dict) or not str(record.get("event", "")).startswith("system."):
                    continue
                safe = {key: record[key] for key in
                        ("completed", "total", "reused_count", "new_count", "schema_valid", "stage_elapsed_seconds")
                        if key in record}
                logger.emit("system.remote.progress", remote_event=record["event"],
                            remote_timestamp_utc=record.get("timestamp_utc"),
                            remote_elapsed_seconds=record.get("elapsed_seconds"), **safe)
    except ClientError as error:
        code = error.response.get("Error", {}).get("Code")
        if code == "ResourceNotFoundException":
            return
        if code == "AccessDeniedException":
            logger.emit("system.progress.status_only", reason="CloudWatch read permission denied; job status monitoring continues")
            return
        raise
...
def execute_system(
    ...
    logger.emit("system.cost_guard", hourly_usd_ceiling=hourly_usd_ceiling,
                estimated_training_usd=estimate, maximum_training_usd=maximum_training_usd,
                scope="Per attempt only; not an AWS account billing cap")
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=root, text=True).strip():
        raise ValueError("Commit source changes before launching or attaching to immutable GPU work")
    commit = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=root, text=True).strip()
    source = subprocess.check_output(
        ["git", "archive", "--format=tar.gz", "HEAD", "src", "configs", "pipelines", "scripts", "uv.lock", "pyproject.toml"],
        cwd=root,
    )
    ...
    request = training_request(root, manifest, bucket, region, role, attempt, source_key, source_sha, commit)
    ...
    if stored_request is not None:
        ...
        verify_training_quota(service_quotas=session.client("service-quotas"),
                              instance_type="ml.g6e.2xlarge", instance_count=1, managed_spot=False)
        store.write(request_key, request)
        if store.read(request_key) != request:
            raise ValueError("Launch intent failed durable read-back")
    description = ensure_job(client, request, acknowledge_charges=acknowledge_charges,
                             allow_retry=allow_retry, attempt=attempt, logger=logger)
    ...
    while description["TrainingJobStatus"] not in TERMINAL:
        ...
        logger.emit("system.job.heartbeat", job_name=request["TrainingJobName"],
                    status=description["TrainingJobStatus"], secondary_status=description.get("SecondaryStatus"),
    ...
    stream_progress(logs, request["TrainingJobName"], logger, seen_events)
    receipt = {
        "job_name": request["TrainingJobName"], "status": description["TrainingJobStatus"],
    ...
    if receipt["status"] != "Completed":
        raise RuntimeError("GPU attempt did not complete. Saved answers remain reusable; a new attempt needs ATTEMPT and RETRY=YES. No automatic paid retry.")
