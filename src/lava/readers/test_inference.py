...
from lava.evaluation.submission import SubmissionInputs, build_submission, load_submission_inputs, validate_submission_csv
...
from lava.readers.system import put_blob, select_pages, store_for, system_contract, validate_prediction
...
def submission_inputs(root: Path, s3: Any, bucket: str, logger: RuntimeEventLogger) -> SubmissionInputs:
...
def test_sources(root: Path, inputs: SubmissionInputs, contract: dict[str, Any]) -> dict[str, dict[str, str]]:
    ...
    for row in csv.DictReader(io.StringIO(payload.decode("utf-8-sig"))):
        ...
        sources[document] = {"key": row["s3_key"], "sha256": row["sha256"], "version_id": row["version_id"]}
...
def validate_test_manifest(manifest: dict[str, Any], contract: dict[str, Any]) -> tuple[TestReaderInput, ...]:
    ...
    if (
        ...
        or any(len(row.pages) != min(counts[row.document_id], contract["config"]["page_budget"]) for row in requests)
...
def render_test_asset(s3: Any, bucket: str, payload: bytes, source: dict[str, str], document: str,
                      alias: str, number: int, render: dict[str, Any], identity: dict[str, Any]) -> dict[str, Any]:
    ...
    fields: dict[str, Any] = {}
    for kind, suffix, data, mime in (("image", "png", image, "image/png"), ("text", "txt", text, "text/plain"), ("layout", "json", layout, "application/json")):
    ...
    return TestPageAsset(
        asset_version="test-retrieved-pages-v1", document_id=document, document_alias=alias,
        page_number=number, source_pdf_s3_uri=f"s3://{bucket}/{source['key']}",
        source_pdf_sha256=source["sha256"], source_pdf_version_id=source["version_id"],
        width_pixels=width, height_pixels=height, dpi=render["dpi"],
        native_text_characters=len(text.decode()), word_count=words,
        text_block_count=blocks, embedded_image_count=images, **fields,
    ).model_dump(mode="json")


def prepare_test_inputs(root: Path, s3: Any, bucket: str, logger: RuntimeEventLogger) -> dict[str, Any]:
    ...
    renderer = yaml.safe_load((root / "configs/oracle_reader_benchmark.yaml").read_bytes())["asset_builder"]
    ...
    for index, (document, questions) in enumerate(sorted(grouped.items()), 1):
        ...
        payload = cached_source(s3, bucket, root / "artifacts/submission/inputs" / f"{document}.pdf", source, logger)
        extraction, _ = stage(retrieval_store, {"implementation": retrieval_id, "source": source}, partial(extract_pages, payload), logger=logger, name="documents")
        ...
        for question in questions:
            ...
            ranking, _ = stage(retrieval_store, {"implementation": retrieval_id, "query": asdict(query), "pages_sha256": digest(encode([asdict(page) for page in pages]))}, partial(rank_query, query, pages, contract["retrieval"]["config"]["bm25"]), logger=logger, name="queries")
            ...
            for number in selected:
                if number not in assets:
                    identity = {"source": source, "render": render, "page": number, "alias": alias,
                                "implementation": contract["source_sha256"]["src/lava/readers/test_inference.py"],
                                "pymupdf": contract["retrieval"]["pymupdf_version"]}
                    asset, _ = stage(store, identity, partial(render_test_asset, s3, bucket, payload, source, document, alias, number, render, identity), logger=logger, name="pages")
                    assets[number] = TestPageAsset.model_validate(asset)
            request = TestReaderInput(question_id=query.question_id, document_id=document, document_alias=alias, question=query.question, answer_format=AnswerFormat(question["answer_format"]), language=question["language"], pages=tuple(assets[n] for n in selected))
            requests.append(request.model_dump(mode="json"))
            logger.emit("export.input.prepared", completed=len(requests), total=len(inputs.order), documents_completed=index-1)
    requests.sort(key=lambda row: row["question_id"])
    manifest = {"schema_version": 1, "contract": contract, "inputs": requests, "inputs_sha256": digest(encode(requests)), "page_counts": counts, "page_counts_sha256": digest(encode(counts))}
...
def test_training_request(*args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    request["TrainingJobName"] = request["TrainingJobName"].replace("lava-system-", "lava-export-", 1)
    request["OutputDataConfig"]["S3OutputPath"] = request["OutputDataConfig"]["S3OutputPath"].replace("/system-runs/", "/test-runs/")
    request["AlgorithmSpecification"]["ContainerArguments"][1] = request["AlgorithmSpecification"]["ContainerArguments"][1].replace("/oracle_reader/run.sh", "/submission/run.sh")
    request["Environment"]["PYTHONPATH"] = "/opt/ml/code/src"
    request["Tags"][1]["Value"] = "user-test-export"
    return request


def execute_test(root: Path, session: Any, bucket: str, region: str, logger: RuntimeEventLogger, *, acknowledge_charges: str, attempt: int = 1, allow_retry: bool = False, maximum_training_usd: float = 15.0) -> dict[str, Any]:
    """User-authorized single GPU attempt; $15 is an estimate guard, not an account cap."""
    return execute_prepared(root, session, bucket, region, logger, contract=test_contract(root),
        request_factory=test_training_request, manifest_validator=validate_test_manifest,
        receipt_directory="artifacts/submission/runtime", acknowledge_charges=acknowledge_charges,
        attempt=attempt, allow_retry=allow_retry, maximum_training_usd=maximum_training_usd)
...
def infer_test(root: Path, s3: Any, bucket: str, identity: str, logger: RuntimeEventLogger) -> None:
    ...
    reader = build_reader(load_resolved_model(root / "configs/oracle_reader_models.lock.json", contract["config"]["model_key"]), region=os.environ.get("AWS_DEFAULT_REGION", "us-west-2"))
    records, reused = [], 0
    for number, request in enumerate(requests, 1):
        ...
        if saved is None:
            with logger.stage("system.question.inference", heartbeat_seconds=15):
                ...
                saved = {"contract_id": identity, "input_sha256": digest(encode(request.model_dump(mode="json"))), "prediction": prediction.model_dump(mode="json"), "telemetry": telemetry.model_dump(mode="json"), "raw_response": read_raw_response(request.question_id), "inference_code_commit": os.environ.get("LAVA_GIT_COMMIT_SHA")}
        ...
        logger.emit("system.question.completed", completed=number, total=len(requests), reused_count=reused, schema_valid=prediction.schema_valid)
    result = {"schema_version": 1, "contract_id": identity, "inputs_sha256": manifest["inputs_sha256"], "records": records}
    ...
    logger.emit("system.job.completed", completed=len(records), reused_count=reused, uploaded_to_kaggle=False)
...
def export_test(root: Path, s3: Any, bucket: str, logger: RuntimeEventLogger) -> Path:
    ...
    requests = validate_test_manifest(manifest, contract)
    if inference.get("contract_id") != contract["contract_id"] or inference.get("inputs_sha256") != manifest["inputs_sha256"] or len(inference["records"]) != len(requests):
    ...
    for request in requests:
        ...
        if original is None or any(
            ...
            for field, column in (("document_id", "file_id"), ("question", "question"),
    ...
    for request, saved in zip(requests, inference["records"], strict=True):
        ...
        predictions.append({"question_id": request.question_id, "answer": prediction.answer, "evidence_pages": list(prediction.evidence_pages)})
    ...
    if invalid:
        ...
        atomic_write(root / "artifacts/submission/invalid_questions.json", encode({"question_ids": invalid}))
        raise ValueError(f"{len(invalid)} invalid/abstained answers require review; no export created")
    ...
    provenance = {"split": "test", "evidence_scope": "retrieved_full_document_pages", "test_csv_sha256": inputs.source_sha256["test.csv"], "question_count": len(inputs.order), "model_id": contract["model"]["model_id"], "model_revision": contract["model"]["revision"], "code_commit": min(commits), "inference_code_commits": sorted(commits), "code_commit_semantics": "representative revision; all compatible revisions are recorded", "predictions_sha256": digest(payload), "page_counts_sha256": manifest["page_counts_sha256"], "inference_contract": contract["contract_id"]}
    ...
    output_manifest = {"schema_version": 1, "competition": contract["submission"]["competition"], "source_sha256": inputs.source_sha256, "prediction_sha256": digest(payload), "provenance": provenance, "submission_sha256": digest(csv_payload), "validation": validation}
    target = persist_submission(s3, bucket, root, csv_payload, output_manifest, logger)
    for name, data in (("predictions.jsonl", payload), ("page_counts.json", encode(manifest["page_counts"])), ("provenance.json", encode(provenance))):
    ...
    logger.emit("export.download.ready", filename="artifacts/submission/submission.csv", rows=len(inputs.order), sha256=digest(csv_payload), uploaded_to_kaggle=False)
    return target
...
if __name__ == "__main__":
    ...
    with log.stage("system.job", heartbeat_seconds=15):
        infer_test(root, boto3.client("s3", region_name=os.environ["AWS_DEFAULT_REGION"]), os.environ["LAVA_BUCKET"], os.environ["LAVA_SYSTEM_CONTRACT"], log)
